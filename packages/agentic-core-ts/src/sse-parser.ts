/** SSE parser for the Agentic Core streaming API. */

import type { AgenticStreamEvent } from "./events.js";
import { AgenticStreamParseError } from "./errors.js";

/**
 * Parse an SSE text/event-stream into an async iterable of typed events.
 *
...
function parseFrame(frame: string): AgenticStreamEvent | null {
  ...
  switch (eventName) {
    case "status": {
      ...
      return { type: "status", stage: parsed.stage ?? "", tool: parsed.tool };
    }
    case "token": {
      const parsed = safeJSON(data);
      return { type: "token", delta: tokenDelta(parsed) };
    }
    case "tool_call": {
      const parsed = safeJSON(data);
      return {
    ...
    case "citations": {
      const parsed = safeJSON(data);
      return { type: "citations", citations: citationList(parsed) };
    }
    case "done":
      return { type: "done" };
    case "error": {
